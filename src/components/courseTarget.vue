    ...
    <div>
    	<div v-for="item in ranklist" class="selectL">
    		...
    		<div class="selectD">
    			...
    			<span :style="`position:absolute;top:0;left:0;z-index:-1;height:100%;width:0.05rem;width:${item.target*100/(item.width*2)>100?100:item.target*100/(item.width*2)}%;background:#bdb8b8;display:inline-block;`"></span><!-- 灰色条 -->
    			<span :style="`position:absolute;top:0;height:100%;width:0.05rem;background:black;left:${item.target*100/(item.width*2)>100?100:item.target*100/(item.width*2)}%;`"></span><!-- 黑色线 -->
...
export default {
  ...
  data () {
    return {
    	ranklist:[{no:1,img:'../../static/image/test.png',name:'王飞雪',width:10,target:18,desc:'已完成10,(占比65%)'},
    	{no:1,img:'../../static/image/test.png',name:'王雪',width:32,target:15,desc:'已完成32,(占比165%)'},
    	{no:1,img:'../../static/image/test.png',name:'李薇薇',width:22,target:25,desc:'已完成22,(占比85%)'},
    	{no:1,img:'../../static/image/test.png',name:'王可可',width:12,target:35,desc:'已完成12,(占比35%)'},],
      choose:1,
      name1:'总课耗量指标',
      name2:'已完成指标',
